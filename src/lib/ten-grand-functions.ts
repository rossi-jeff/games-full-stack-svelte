import { TenGrandCategory, TenGrandDiceRequired } from './enum/ten-grand-category.enum';
...
export const getCategoryScoreAndDice = (category: TenGrandCategory | undefined, dice: number[]) => {
	let Score = 0;
	let Used: number[] = [];
	switch (category) {
		case 'Ones':
...
export const sortByDicerequired = (a: TenGrandOption, b: TenGrandOption) =>
	(TenGrandDiceRequired[b.Category ?? ''] ?? 0) - (TenGrandDiceRequired[a.Category ?? ''] ?? 0);
